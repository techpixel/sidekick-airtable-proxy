...
**Each record in our base is a ship.** We don't have any re-ships. This means that the timeline ALWAYS only holds one event: the ship event. Ship status comes from the `Status` single-select: `Approved` -> approved, `Rejected` -> rejected, anything else is a pending ship.

**An approval MODIFIES the record, instead of creating a new one.** We also don't show any public-facing user feedback, so feel free to discard whatever the user wrote in the public feedback field (and always show a constant string like "(none)" when sending data back to Sidekick for the public feedback fields). Only the internal justification matters (which gets written to `Justification`). An approval sets `Status` to `Approved` — it does NOT touch `Automation - Submit to Unified YSWS`; submitting to Unified is handled separately.
...
When constructing the description for projects, include `Description`, `GitHub Username`, **AND** `Original Hours` (add an epilogue like "GitHub: <USERNAME>" and "Author originally logged <HOURS>").

A rejection sets `Status` to `Rejected`.
